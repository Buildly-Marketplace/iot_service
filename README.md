...
```bash
docker-compose run --rm --entrypoint 'bash scripts/run-tests.sh' iot_service
```
...
```bash
docker-compose run --rm --entrypoint 'bash scripts/run-tests.sh --bash-on-finish' iot_service
```

To run bash:

```bash
docker-compose run --rm --entrypoint 'bash' iot_service
```
